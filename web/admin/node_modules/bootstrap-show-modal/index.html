...
<section class="container">
    ...
    <div class="pb-2">
        <h3>Dark themed Alert (With "OK" Button)</h3>
    ...
    <div class="pb-2">
        <h3>Confirmation dialog with custom button classes</h3>
        <pre><code class="language-javascript">bootstrap.showConfirm({
    title: "Delete file?", body: "This cannot be undone.",
    textTrue: "Delete", classTrue: "btn btn-danger",
    textFalse: "Cancel", classFalse: "btn btn-outline-secondary",
    onSubmit: function (result) {
        bootstrap.showAlert({title: "Result: " + result, body: result ? "Deleted." : "Canceled."})
    }
})</code></pre>
        <button type="button" id="button-confirm-danger" class="btn btn-primary">Show Confirm with custom buttons</button>
    </div>
    <div class="pb-2">
...
<script type="module">
    ...
    document.getElementById("button-alert").addEventListener("click", function () {
        bootstrap.showAlert({title: "Hi", body: "Please press ok, if you like or dislike cookies.", theme: "dark"})
    })
    ...
    document.getElementById("button-confirm-danger").addEventListener("click", function () {
        bootstrap.showConfirm({
            title: "Delete file?", body: "This cannot be undone.",
            textTrue: "Delete", classTrue: "btn btn-danger",
            textFalse: "Cancel", classFalse: "btn btn-outline-secondary",
    ...
    document.getElementById("button-form").addEventListener("click", function () {
        bootstrap.showModal({
            ...
            body:
                ...
                '<div class="form-group row mb-2">' +
                '<div class="col-3"><label for="select" class="col-form-label">Select</label></div>' +
                '<div class="col-9"><select id="select" class="form-select">' +
                '<option value=""></option>' +
                '<option value="red">red</option><option value="green">green</option><option value="blue">blue</option>' +
                '</select></div>' +
                '</div>' +
